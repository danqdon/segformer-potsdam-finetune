...
def create_input_transform(image, label):
    ...
    try:
        ...
        if "pixel_values" not in encoded_inputs or "labels" not in encoded_inputs:
            ...
            pv = encoded_inputs.get("pixel_values"); lbl = encoded_inputs.get("labels")
            logging.error(f"Contents: pixel_values type={type(pv)}, labels type={type(lbl)}")
            return None
...
def run_training(arguments):
    ...
    for epoch in range(arguments.epochs):
        ...
        try:
            for batch_data in train_progress_bar:
                ...
                if pixel_values.ndim == 5 and pixel_values.shape[1] == 1: pixel_values = pixel_values.squeeze(1)
                elif pixel_values.ndim != 4: raise ValueError(f"Unexpected training pixel_values dimension: {pixel_values.ndim}")

                if target_labels.ndim == 4 and target_labels.shape[1] == 1: target_labels = target_labels.squeeze(1)
                elif target_labels.ndim != 3: raise ValueError(f"Unexpected training target_labels dimension: {target_labels.ndim}")

                target_labels = target_labels.long()
                pixel_values = pixel_values.to(config.DEVICE); target_labels = target_labels.to(config.DEVICE)

                optimizer.zero_grad()
                model_outputs = model(pixel_values=pixel_values, labels=None)
                output_logits = model_outputs.logits
                upsampled_logits = nn.functional.interpolate(output_logits, size=target_labels.shape[-2:], mode="bilinear", align_corners=False)
                loss = loss_criterion(upsampled_logits, target_labels)
                loss.backward()
                optimizer.step()
                if learning_rate_scheduler: learning_rate_scheduler.step()
                total_train_loss += loss.item()
                train_progress_bar.set_postfix(loss=loss.item())
        except Exception as e:
             logging.error(f"Error during training loop iteration: {e}", exc_info=True); raise e
        ...
        try:
            with torch.no_grad():
                for batch_data in validation_progress_bar:
                    ...
                    pixel_values = batch_data["pixel_values"]; target_labels = batch_data["labels"]

                    if pixel_values.ndim == 5 and pixel_values.shape[1] == 1: pixel_values = pixel_values.squeeze(1)
                    elif pixel_values.ndim != 4: raise ValueError(f"Unexpected validation pixel_values dimension: {pixel_values.ndim}")
                    if target_labels.ndim == 4 and target_labels.shape[1] == 1: target_labels = target_labels.squeeze(1)
                    elif target_labels.ndim != 3: raise ValueError(f"Unexpected validation target_labels dimension: {target_labels.ndim}")
            ...
            logging.error(f"Error during validation loop iteration: {e}", exc_info=True); raise e

        average_validation_loss = total_validation_loss / len(valid_dataloader) if len(valid_dataloader) > 0 else 0

        validation_miou = 0.0; validation_mean_accuracy = 0.0; per_category_iou = [0.0] * config.NUM_CLASSES
        try:
        ...
        if (epoch + 1) % config.CHECKPOINT_SAVE_INTERVAL_EPOCHS == 0:
            ...
            checkpoint_path = config.MODEL_SAVE_DIR / checkpoint_filename
            try: torch.save(model.state_dict(), checkpoint_path); logging.info(f"Saved epoch checkpoint: {checkpoint_path}")
        ...
        if validation_miou > best_validation_miou:
            ...
            epochs_without_miou_improvement = 0
            best_model_state_dict_path = config.BEST_MODEL_STATE_DICT_PATH
            try:
                torch.save(model.state_dict(), best_model_state_dict_path)
                logging.info(f"New best mIoU: {validation_miou:.4f}. Saved best model state_dict to {best_model_state_dict_path}")
                try: 
                    ...
                    with open(config.BEST_METRICS_CSV_PATH, "w", newline="") as csvfile:
                        csv_writer = csv.writer(csvfile); csv_writer.writerow(csv_header); csv_writer.writerow(csv_row)
                    logging.info(f"Saved best metrics to {config.BEST_METRICS_CSV_PATH}")
                except Exception as e: logging.error(f"Failed to save best metrics CSV: {e}")
            except Exception as e: logging.error(f"Failed to save best model state_dict {best_model_state_dict_path}: {e}")
        else:
    ...
    try:
        ...
        final_model_save_path = config.MODEL_SAVE_DIR / arguments.output_model_name
    ...
    except Exception as e: logging.error(f"Failed to save final model: {e}")
